...
//                .within(Time.seconds(10));
        ...
        SingleOutputStreamOperator<JSONObject> selectDS = patternStream.select(timeOutTag,
                /*PatternTimeoutFunction cep超时的时候, 调用的函数*/
                new PatternTimeoutFunction<JSONObject, JSONObject>() {
                    /*
                    map.get("first")返回 是一个List<JSONObject>>??
                    因为cep你可以定义begin是first, second也是first, 这个时候first时间在1个匹配单元中, 就有2个元素了;
                     */
                    @Override
                    public JSONObject timeout(Map<String, List<JSONObject>> map, long l) throws Exception {
                        return map.get("first").get(0);
                    }
                    /*PatternSelectFunction cep匹配成功的时候, 调用的函数*/
        ...
        //TODO 9.合并两个事件流
        /*流的合并方式案例 union 相当于SQL中的union all*/
