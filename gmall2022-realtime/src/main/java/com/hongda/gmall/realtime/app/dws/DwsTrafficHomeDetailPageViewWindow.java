...
import com.hongda.gmall.realtime.bean.TrafficHomeDetailPageViewBean;
...
//        });
        ...
        SingleOutputStreamOperator<TrafficHomeDetailPageViewBean> trafficHomeDetailDS = keyedStream.flatMap(new RichFlatMapFunction<JSONObject, TrafficHomeDetailPageViewBean>() {
            ...
            public void flatMap(JSONObject value, Collector<TrafficHomeDetailPageViewBean> out) throws Exception {
                ...
                //封装JavaBean并写出数据
                if (homeUvCt != 0L || detailUvCt != 0L) { /*使用flatmap的原因, 因为可能不输出任何元素*/
        ...
        reduceDS.print(">>>>>>>>>>");
        reduceDS.addSink(MyClickHouseUtil.getClickHouseSink("insert into dws_traffic_home_detail_page_view_window values(?,?,?,?,?)"));
