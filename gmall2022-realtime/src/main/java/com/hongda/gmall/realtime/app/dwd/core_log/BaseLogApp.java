...
//        DataStreamSource<String> kafkaDS = env.fromSource(MyKafkaUtilHongda.getKafkaSource("ods_log", "base_log_app_2022"), WatermarkStrategy.noWatermarks(), "ods_log_Source");
        ...
        SingleOutputStreamOperator<String> pageDS = jsonObjWithNewFlagDS.process(new ProcessFunction<JSONObject, String>() {
            ...
            public void processElement(JSONObject value, Context ctx, Collector<String> out) throws Exception {
                ...
                if (start != null) {
                    //输出数据到启动日志
                    ctx.output(startTag, jsonString); //侧输出流输出
                } else {
                    ...
                    if (displays != null && displays.size() > 0) {
                        for (int i = 0; i < displays.size(); i++) {
                            ...
                            ctx.output(displayTag, display.toJSONString()); //侧输出流输出
                        }
                    }
                    /*
                    炸开json数组, 并传送到下游
                    ...
                    if (actions != null && actions.size() > 0) {
                        for (int i = 0; i < actions.size(); i++) {
                            ...
                            ctx.output(actionTag, action.toJSONString());//侧输出流输出
                        }
                    }
                    ...
                    out.collect(value.toJSONString()); //主流输出
                }
            }
        });
